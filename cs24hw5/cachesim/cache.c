...
unsigned char cache_read_byte(membase_t *mb, addr_t address) {
    ...
    /* Update access team for (f) */
    p_line->last_access = clock_tick();
 ...
 *
 * Note:  This functionality is also provided by decompose_address().  The
 *        reason for this redundancy is that decompose_address() is used in
 *        a few specific cases, but this operation is used in quite a few
 *        places, so it makes more sense to break this out as a separate
 *        operation.
 */
addr_t get_offset_in_block(cache_t *p_cache, addr_t address) {
    /* Define the 32-bit bitmask and return variable*/
    uint32_t bitmask;

    /* Set bitmask value, bottom bytes all set to 1 */
    bitmask = p_cache->block_size - 1;

    /* We can derive the original value via a bitwise AND w/ 1 */
...
void decompose_address(cache_t *p_cache, addr_t address,
    ...
    /* We can use the function we just wrote above to set the offset value
     * and we can use right shifts on the adress to get the set/tag vals */
 ...
 */
addr_t get_block_start_from_address(cache_t *p_cache, addr_t address) {
    /* The block start will be the address, just with the in block offset = 0.
     * We do this by shifting right, then left by the offset bits, to get the
     * leading zeros. First, however, we must use an assert statement to
     * ensure that p_cache is not null.
     */
    assert(p_cache);
...
addr_t get_block_start_from_line_info(cache_t *p_cache,
                                      addr_t tag, addr_t set_no) {
    /* In order to get the block start from the line info, we will shift the
     * tag left by sets_addr_bits, then the number from set_no. then we will
     * shift left again by the in block offset again.
     */
...
cacheline_t * find_line_in_set(cacheset_t *p_set, addr_t tag) {
    ...
    /* set the cacheline lines that we will iterate through */
    cacheline_t *lines = p_set->cache_lines;

    /* iterate through lines */
    for (int i = 0; i < p_set->num_lines; i++) {
        /* check each line against tag, and also make sure it's a valid line */
        if ((lines[i].tag == tag) && lines[i].valid) {
            /* now set the found_line & break */
            found_line = &lines[i];
            break;
        }
    }
...
cacheline_t * choose_victim(cacheset_t *p_set) {
...
#else
    /* Time variable (LRU) to hold the last access time, and initialize
     * victim index to -1 to know if we do not ever find one
     */
    uint32_t lru = clock_tick();
    i_victim = -1;
    ...
    /* iterate through lines */
    for (int i = 0; i < p_set->num_lines; i++) {
        /* Look to see the most recent current line access time and
         * compare to last_access */
        if (lines[i].last_access < lru) {
            /* new last_access from last access */
            lru = lines[i].last_access;
            /* set victim index */
            i_victim = i;
        }
    }

    /* Check to see if the victim index is still -1, meaning that no cache line
     * was accessed
     */
    if (i_victim == -1) {
        abort();
    ...
    /* Calculate victim address using the lines value and adding i_victim */
    victim = lines + i_victim;
